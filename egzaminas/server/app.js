...
//CREATE
app.post("/server/meals", (req, res) => {
    const sql = `
    INSERT INTO patiekalai (title, description, price, image)
    VALUES (?, ?, ?, ?)
    `;
    con.query(sql, [req.body.title, req.body.description, req.body.price, req.body.image], (err, result) => {
        if (err) throw err;
        res.send(result);
    });
});
...
//EDIT
app.put("/server/meals/:id", (req, res) => {
    let sql;
    let r;
    if (req.body.deletePhoto) {
        sql = `
        UPDATE patiekalai
        SET title = ?, description = ?, price = ?, image = null
        WHERE id = ?
        `;
        r = [req.body.title, req.body.description, req.body.price, req.params.id];
        ...
        WHERE id = ?
        `;
        r = [req.body.title, req.body.description, req.body.price, req.body.image, req.params.id];
    } else {
        sql = `
        UPDATE patiekalai
